...
struct Vars2 {
	...
	RCP<Belos::LinearProblem<scalar_type, vec_type, op_type>> problem;
...
MueLuWrapper::MueLuWrapper(Mat A, double tol, string config)
{
	...
	vars->problem
	= rcp(new Belos::LinearProblem<scalar_type, vec_type, op_type>(
	...
	vars->solver
	= rcp(new Belos::BlockGmresSolMgr<scalar_type, vec_type,op_type >(
