...
void TriDiagSolver::solve(Domain &dom)
{
	...
	if (dom.hasLeftNbr()) {
		top_left_coeff = -2.0;
		left_boundary  = dom.leftNbr().uCurr().back();
	}
	double right_boundary     = 2 * right_bnd;
	double bottom_right_coeff = -3.0;
	if (dom.hasRightNbr()) {
		bottom_right_coeff = -2.0;
		right_boundary     = dom.rightNbr().uPrev().front();
	}

	// first value
	c_prime[0] = 1.0 / top_left_coeff;
	d_prime[0] = (uxx.at(0) * h * h - left_boundary) / top_left_coeff;
	// in between values
	...
	d_prime[last_i] = (uxx.at(last_i) * h * h - right_boundary - d_prime[last_i - 1])
