...
void DomainCollection::solveWithInterface(double* gamma_raw, double* diff_raw)
{
	...
	vector_type      local_diff(trilObjs->collection_map, 1);
...
double DomainCollection::fNorm()
{
	...
	double retval;
	Teuchos::reduceAll<int, double>(*trilObjs->comm, Teuchos::REDUCE_SUM, 1, &result, &retval);
	return sqrt(retval);
}
...
{
	vector_type ghost(trilObjs->collection_map, 2);
	vector_type one_ghost(trilObjs->matrix_map, 2);
	for (auto &p : domains) {
		p.second->putGhostCells(ghost);
	}
	Tpetra::Export<> exporter(trilObjs->collection_map, trilObjs->matrix_map);
	...
	double retval;
	Teuchos::reduceAll<int, double>(*trilObjs->comm, Teuchos::REDUCE_SUM, 1, &residual, &retval);
	return sqrt(retval);
}
double DomainCollection::integrateF()
{
	...
	double retval;
	Teuchos::reduceAll<int, double>(*trilObjs->comm, Teuchos::REDUCE_SUM, 1, &sum, &retval);
	return retval;
}
...
AmgxCrs DomainCollection::formCRSMatrix()
{
