...
int main(int argc, char *argv[]) {
    ...
    args::HelpFlag help(parser, "help", "Display this help menu",
                        {'h', "help"});
    args::ValueFlag<int> f_n(
        parser, "n", "number of cells in the x direction, in each domain",
        {'n'});
    args::ValueFlag<int> f_l(
        parser, "n", "run the program n times and print out the average",
        {'l'});
    args::Flag f_wrapper(parser, "wrapper", "use a function wrapper", {"wrap"});
    args::ValueFlag<int> f_div(parser, "divide", "use iterative method",
                               {"divide"});
    args::Flag f_nozerof(parser, "zerou",
                         "don't  make the rhs match the boundary conditions",
                         {"nozerof"});
    args::ValueFlag<double> f_t(
        parser, "tolerance",
        "set the tolerance of the iterative solver (default is 1e-10)", {'t'});
    args::Flag f_neumann(parser, "", "use neumann boundary conditions",
                         {"neumann"});
    args::Flag f_noschur(parser, "", "don't use schur compliment method",
                         {"noschur"});

    // mesh options
    args::ValueFlag<string> f_mesh(parser, "file_name", "read in a mesh",
                                   {"mesh"});
    ...
    // output options
    args::Flag f_outclaw(parser, "outclaw", "output amrclaw ascii file",
                         {"outclaw"});
#ifdef HAVE_VTK
    args::ValueFlag<string> f_outvtk(parser, "", "output to vtk format",
                                     {"outvtk"});
#endif
    args::ValueFlag<string> f_m(parser, "matrix filename",
                                "the file to write the matrix to", {'m'});
    args::ValueFlag<string> f_s(parser, "solution filename",
                                "the file to write the solution to", {'s'});
    args::ValueFlag<string> f_resid(parser, "residual filename",
                                    "the file to write the residual to",
                                    {"residual"});
    args::ValueFlag<string> f_error(
        parser, "error filename", "the file to write the error to", {"error"});
    args::ValueFlag<string> f_rhs(parser, "rhs filename",
                                  "the file to write the rhs vector to", {'r'});
    args::ValueFlag<string> f_g(parser, "gamma filename",
                                "the file to write the gamma vector to", {'g'});
    args::ValueFlag<string> f_read_gamma(parser, "gamma filename",
                                         "the file to read gamma vector from",
                                         {"readgamma"});
    ...
    // patch solvers
    args::Flag f_fish(parser, "fishpack", "use fishpack as the patch solver",
                      {"fishpack"});
#ifdef __NVCC__
    args::Flag f_cufft(parser, "cufft", "use CuFFT as the patch solver",
                       {"cufft"});
#endif

// third-party preconditioners
#ifdef ENABLE_AMGX
    args::ValueFlag<string> f_amgx(
        parser, "", "solve schur compliment system with amgx", {"amgx"});
#endif
#ifdef ENABLE_MUELU
    args::ValueFlag<string> f_meulu(
        parser, "", "solve schur compliment system with muelu", {"muelu"});
#endif
#ifdef ENABLE_MUELU_CUDA
    args::ValueFlag<string> f_meulucuda(
        parser, "", "solve schur compliment system with muelu", {"muelucuda"});
#endif
    ...
    if (f_zero) {
        ...
        ffun = [](double x, double y) {
            return 100 * M_PI * M_PI *
                       (pow(sin(10 * M_PI * x), 2) - cos(10 * M_PI * x)) *
                       exp(cos(10 * M_PI * x)) +
                   121 * M_PI * M_PI *
                       (cos(11 * M_PI * y) - pow(sin(11 * M_PI * y), 2)) *
                       exp(cos(11 * M_PI * y));
        };
        ...
        gfun = [](double x,
                  double y) { return sinl(M_PI * y) * cosl(2 * M_PI * x); };
        nfun = [](double x, double y) {
    ...
    for (int loop = 0; loop < loop_count; loop++) {
        ...
        if (f_noschur || dc.num_global_domains != 1) {
            ...
            if (false) {
                ...
                timer.start("MeuLu Setup");
                meulucudasolver =
                    new MueLuCudaWrapper(A, tol, args::get(f_meulucuda));
                timer.stop("MeuLu Setup");
#endif
#ifdef ENABLE_AMGX
            } else if (f_amgx) {
        ...
        if (f_g) {
            PetscViewer viewer;
            PetscViewerASCIIOpen(PETSC_COMM_WORLD, args::get(f_g).c_str(),
                                 &viewer);
