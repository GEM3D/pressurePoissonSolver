 ...
 *
 * The problem to be solved is the poisson equation where u = sin(pi*x)*sin(pi*y)
...
int main (int argc, char *argv[])
{
   ...
   HYPRE_SStructGridSetExtents(grid, myid, ilower, iupper);
  ...
  //Set dirchelet boundary condition
  {
      int bc_ilower[2];
      int bc_iupper[2];
      int nentries = 1;
      ...
      {  
       ...
       {  
         ...
         HYPRE_SStructMatrixSetBoxValues(A, part, bc_ilower, bc_iupper, var, nentries,stencil_indices, valuesy);
   ...
   for (j = 0; j < ny; j++)
   {
       ...
       for (i = 0; i < nx; i++)
       {
           ...
           rhs_values[i + j * nx] = -8. * PI2 * sin(2 * M_PI * x) * sin(2 * M_PI * y);
   ...
   HYPRE_SStructVectorPrint("IJ.out.b", b, 0);
   ...
   //HYPRE_SStructMatrixPrint(A, "/ExactIJ/IJ.out.A");
   //HYPRE_SStructVectorPrint(b, "/ExactIJ/IJ.out.b");
...
//       char filename[255];
       ...
       int objecttype = HYPRE_SSTRUCT;
       //HYPRE_ParVectorSetObjectType(par_x, objecttype);
       //HYPRE_SStructVectorGetObject(par_x, (void **) &x);
       //HYPRE_SStructVectorGetBoxValues(par_x, part, ilower, iupper, var, values);
       
       /* Calcluate the local L2 norm */
       for (i = 0; i < nvalues; i++)
       { 
          diff = values[i] - exactsolution[i];
          diff2 = diff * diff;
          sum += diff2;
       }
       L2 = sqrt(sum/nvalues);
       ...
       /*
       sprintf(filename, "%s.%06d", "vis/IJ.sol", myid);
...
//      fclose(file);
      ...
      if (myid == 0)
      {
          //GLVis_PrintGlobalSquareMesh("vis/IJ.mesh", n-1);
      }
   }    
